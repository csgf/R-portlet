...
//
public class R_portlet extends GenericPortlet {
    ...
    AppLogger _log = new AppLogger(R_portlet.class);
    ...
    private void getPreferences( ActionRequest actionRequest
        ...
        if (null != prefs) {
            ...
            int numInfras=appPreferences.getNumInfrastructures();  _log.debug(LS+">>>>>>>>>>> numInfras: '"+numInfras+"'"+LS);
            
            // Allocate the array only if it is not initialized yet
            if(appPreferences.infoInfra == null) {          
    ...
    void submitJob(App_Input appInput) {         
	    ...
	    if(appPreferences.infoInfra[i].enableInfrastructure.equalsIgnoreCase("yes")) {
                ...
                if(appPreferences.infoInfra[i].acronymInfrastructure.equalsIgnoreCase("unicore")) {
                    ...
                    String unicoreItems[]  = appPreferences.infoInfra[i].bdiiHost.split(";");
                ...
                else if(appPreferences.infoInfra[i].acronymInfrastructure.equalsIgnoreCase("garuda")) {
                    // Multi-infrastructure support (field mapping)
                    ...
                    if(_log !=null)
                        _log.info("GARUDA infrastructure: "
                               +LS+" pxServerHost:  '" + appPreferences.infoInfra[i].pxServerHost +"'"
                               +LS+" pxServerPort:  '" + appPreferences.infoInfra[i].pxServerPort +"'"
                               +LS+" pxRobotId:     '" + appPreferences.infoInfra[i].pxRobotId +"'"
                               +LS+" VO Group:      '" + appPreferences.infoInfra[i].pxRobotVO +"'"
                               +LS+" VO Role:       '" + appPreferences.infoInfra[i].pxRobotRole +"'"
                               +LS+" JSAGA adaptor: '" + wmsHostList        +"'"
                                 ); 
                }
                else if(appPreferences.infoInfra[i].acronymInfrastructure.equalsIgnoreCase("ourgrid")) {
                    // Multi-infrastructure support (field mapping)
                    // OURGRID infrastructure requires to specify:
                    // * Username      -> bdiiHost first  field of the given ';' separated string
                    // * Password      -> bdiiHost second field of the given ';' separated string
                    // * JSAGA adaptor -> wmsHosts
                    String ourgridItems[]  = appPreferences.infoInfra[i].bdiiHost.split(";");
                ...
                if(_log !=null) 
                    _log.info(LS+appPreferences.infoInfra[i].dumpInfrastructure());
            } // Add enabled infrastructure
            else {
                if(_log !=null)
                    _log.info(LS+"Disabled infrastructure: "
                             +LS+appPreferences.infoInfra[i].dumpInfrastructure());  
            }
        } // for each infrastructure
 
	for(int iEn=0; iEn<infrastructures.length; iEn++)
             miJobSubmission.addInfrastructure(infrastructures[iEn]);
            //
            ...
            String arguments =appPreferences.pilotScript      // executable' argument: pilot_script.sh
                             +" '"+appInput.jobIdentifier+"'" // User job identifier
                             +" '"+appInput.username+"'"      // User name
                             ;                                // 
            String outputPath="/tmp/";                    // Output Path
            String outputFile=".R-Output.txt";      // Distributed application standard output
            String errorFile =".R-Error.txt";       // Distrubuted application standard error
            String appFile   ="R-Files.tar.gz";    // Hostname output files (created by the pilot script)
            ...
            // Submit Job
            miJobSubmission.submitJobAsync(miJobSubmission.getInfrastructure(),username, hostUTDB, applicationId, appInput.jobIdentifier);
